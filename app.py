...
# Replace the integers below with your training label mappings if known
...
# HTML Template with an attractive, responsive UI layout built using TailwindCSS
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="max-w-5xl mx-auto px-4 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500">Advanced XGBoost Diagnostic Engine</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                ...
                <form id="predictionForm" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="number" name="Age" min="0" max="120" value="45" required 
                                   class="w-full px-3.5 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                    ...
                    <button type="submit" 
                            class="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2.5 px-4 rounded-lg transition-colors shadow-md shadow-indigo-100 flex items-center justify-center space-x-2 cursor-pointer">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col justify-between">
                <div>
                    ...
                    <div id="placeholder" class="text-center py-20 text-slate-400">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto mb-3 opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div id="resultsCard" class="hidden space-y-6 animate-fade-in">
                        <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-5 text-center">
                            ...
                            <div id="predictionValue" class="text-3xl font-extrabold text-indigo-900">Class 0</div>
                        </div>
                ...
                <div class="mt-6 pt-4 border-t border-slate-100 text-center text-xs text-slate-400">
                    XGBoost inference pipeline loaded successfully.
                </div>
            </div>
        </div>
    </main>
        ...
        document.getElementById('predictionForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    document.getElementById('predictionValue').innerText = `Cluster Match: ${data.prediction}`;
...
def predict():
    ...
    try:
        ...
        # Structure the data precisely matched to model requirement order
        input_data = {
            "Age": [int(data["Age"])],
            "Gender": [CATEGORICAL_MAPPINGS["Gender"][data["Gender"]]],
            "Blood Type": [CATEGORICAL_MAPPINGS["Blood Type"][data["Blood Type"]]],
            "Medical Condition": [CATEGORICAL_MAPPINGS["Medical Condition"][data["Medical Condition"]]],
            "Hospital": [CATEGORICAL_MAPPINGS["Hospital"][data["Hospital"]]],
            "Insurance Provider": [CATEGORICAL_MAPPINGS["Insurance Provider"][data["Insurance Provider"]]],
            "Billing Amount": [float(data["Billing Amount"])],
            "Admission Type": [CATEGORICAL_MAPPINGS["Admission Type"][data["Admission Type"]]],
            "Medication": [CATEGORICAL_MAPPINGS["Medication"][data["Medication"]]]
        }
        
        df = pd.DataFrame(input_data)
        
        # Enforce exact input features ordering matching original schema
        feature_order = ["Age", "Gender", "Blood Type", "Medical Condition", "Hospital", "Insurance Provider", "Billing Amount", "Admission Type", "Medication"]
        df = df[feature_order]
        
        # Perform calculation predictions
